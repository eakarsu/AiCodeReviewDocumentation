...
// Feature: Security posture scoring aggregating SAST/DAST/dependencies with trending metrics
import React, { useState } from 'react';
import api from '../services/api';

export default function SecurityPostureScoringAggregatingSastDastDepPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-security-posture-scoring-aggregating-sast-dast-dep/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Security posture scoring aggregating SAST/DAST/dependencies with trending metrics</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-security-posture-scoring-aggregating-sast-dast-dep/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
