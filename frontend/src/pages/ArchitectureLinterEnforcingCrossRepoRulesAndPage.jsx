...
// Feature: Architecture linter enforcing cross-repo rules and detecting circular dependencies
import React, { useState } from 'react';
import api from '../services/api';

export default function ArchitectureLinterEnforcingCrossRepoRulesAndPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-architecture-linter-enforcing-cross-repo-rules-and/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Architecture linter enforcing cross-repo rules and detecting circular dependencies</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-architecture-linter-enforcing-cross-repo-rules-and/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
