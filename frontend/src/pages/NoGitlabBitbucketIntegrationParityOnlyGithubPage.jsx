...
export default function NoGitlabBitbucketIntegrationParityOnlyGithubPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-gitlab-bitbucket-integration-parity-only-github/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No GitLab/Bitbucket integration parity (only GitHub)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-gitlab-bitbucket-integration-parity-only-github/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
