...
export default function NoAiPrSummaryOrChangeImpactAnalyzerWiredTPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-ai-pr-summary-or-change-impact-analyzer-wired-t/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No AI PR-summary or change-impact analyzer wired to /api/ai</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-ai-pr-summary-or-change-impact-analyzer-wired-t/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
