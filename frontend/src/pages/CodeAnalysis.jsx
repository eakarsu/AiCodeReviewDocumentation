...
function CodeAnalysis() {
  ...
  const fetchItems = async () => {
    try { const data = await codeAnalysisApi.getAll(); setItems(data); }
  ...
  const handleCreate = async (data) => {
    setFormLoading(true);
    try { await codeAnalysisApi.create(data); setShowNewForm(false); fetchItems(); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await codeAnalysisApi.delete(id); setSelectedItem(null); fetchItems(); }
    catch (err) { alert('Error: ' + err.message); }
  };

  const handleAnalyze = async (id) => {
    setAiLoading(true);
    try { const updated = await codeAnalysisApi.analyze(id); setSelectedItem(updated); fetchItems(); }
    catch (err) { alert('Error: ' + err.message); }
            ...
            {selectedItem.analysis_result && (
              <div><h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Analysis Result</h4>
                <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
                  <pre className="whitespace-pre-wrap text-sm text-purple-800 dark:text-purple-200">{selectedItem.analysis_result}</pre>
                </div>
              </div>
            )}
          </div>
