...
export default function NotificationRoutesExistButNoSlackEmailDelivPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-notification-routes-exist-but-no-slack-email-deliv/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Notification routes exist but no Slack/email delivery channel</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-notification-routes-exist-but-no-slack-email-deliv/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
