...
export default function Critical0MountedChatStyleAiEndpointsDespitePage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-critical-0-mounted-chat-style-ai-endpoints-despite/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>CRITICAL: 0 mounted /chat-style AI endpoints despite the entire domain being AI code review</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-critical-0-mounted-chat-style-ai-endpoints-despite/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
