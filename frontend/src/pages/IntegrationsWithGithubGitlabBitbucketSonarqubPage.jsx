...
// Feature: Integrations with GitHub, GitLab, Bitbucket, SonarQube, and Checkmarx
import React, { useState } from 'react';
import api from '../services/api';

export default function IntegrationsWithGithubGitlabBitbucketSonarqubPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-integrations-with-github-gitlab-bitbucket-sonarqub/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Integrations with GitHub, GitLab, Bitbucket, SonarQube, and Checkmarx</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-integrations-with-github-gitlab-bitbucket-sonarqub/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
