...
export default function NoSbomLicenseComplianceReportingBeyondDependPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-sbom-license-compliance-reporting-beyond-depend/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No SBOM / license-compliance reporting beyond dependency audit</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-sbom-license-compliance-reporting-beyond-depend/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
