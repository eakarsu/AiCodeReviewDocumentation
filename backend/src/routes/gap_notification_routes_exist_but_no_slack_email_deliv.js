...
// Feature: Notification routes exist but no Slack/email delivery channel
  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_notification_routes_exist_but_no_slack_email_deliv',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ 'Notification routes exist but no Slack/email delivery channel' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: 'Notification routes exist but no Slack/email delivery channel'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_notification_routes_exist_but_no_slack_email_deliv', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_notification_routes_exist_but_no_slack_email_deliv', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_notification_routes_exist_but_no_slack_email_deliv', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_notification_routes_exist_but_no_slack_email_deliv']
    );
