...
// Feature: No IDE plugin (VS Code, JetBrains)
  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_no_ide_plugin_vs_code_jetbrains',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ 'No IDE plugin (VS Code, JetBrains)' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: 'No IDE plugin (VS Code, JetBrains)'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_no_ide_plugin_vs_code_jetbrains', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_no_ide_plugin_vs_code_jetbrains', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_no_ide_plugin_vs_code_jetbrains', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_no_ide_plugin_vs_code_jetbrains']
    );
